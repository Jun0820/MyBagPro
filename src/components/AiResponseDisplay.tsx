                ...
                <div className="bg-gradient-to-br from-slate-900 to-trust-navy rounded-[2rem] p-1 border border-slate-700 shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="bg-slate-900/40 backdrop-blur-md rounded-[1.8rem] p-6 md:p-8 relative z-10">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            <a href={getAffiliateUrl('', bestMatch.items['ヘッド'], 'AMAZON')} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 bg-[#FF9900] hover:bg-[#FF9900]/90 text-white font-bold py-3 px-4 rounded-xl transition-all shadow-lg shadow-[#FF9900]/20 active:scale-95 text-[10px] md:text-xs">
                                <ShoppingCart size={16} />
                                Amazon
                            </a>
                            <a href={getAffiliateUrl('', bestMatch.items['ヘッド'], 'RAKUTEN')} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 bg-[#BF0000] hover:bg-[#BF0000]/90 text-white font-bold py-3 px-4 rounded-xl transition-all shadow-lg shadow-[#BF0000]/20 active:scale-95 text-[10px] md:text-xs">
                                <ShoppingCart size={16} />
                                楽天市場
                            </a>
                            <a href={getAffiliateUrl('', bestMatch.items['ヘッド'], 'RAKUTEN_GOLF_PARTNER')} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-4 rounded-xl transition-all shadow-lg shadow-green-600/20 active:scale-95 text-[10px] md:text-xs">
                                <Search size={16} />
                                Golf Partner
                            </a>
                        </div>
                ...
                <div className="bg-gradient-to-br from-amber-50 to-orange-50/30 rounded-[2rem] p-6 md:p-8 border border-amber-200/50 shadow-xl shadow-amber-900/5 relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <a href={getAffiliateUrl('', vintageMatch.items['ヘッド'], 'RAKUTEN_GOLF_PARTNER')} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3.5 px-4 rounded-xl transition-all active:scale-95 text-xs shadow-md">
                                <Search size={18} />
                                Golf Partner 中古在庫
                            </a>
                            <a href={getAffiliateUrl('', vintageMatch.items['ヘッド'], 'AMAZON')} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-white font-bold py-3.5 px-4 rounded-xl transition-all active:scale-95 text-xs shadow-md">
